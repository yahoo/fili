...
public abstract class BaseTableLoader implements TableLoader {
    ...
    public TableGroup buildTableGroup(
        ...
        return new TableGroup(physicalTables, apiMetrics);
    }
     ...
     */
    @Deprecated
    public void loadLogicalTables(
            Map<String, TableGroup> nameGroupMap,
            Set<TimeGrain> validGrains,
            ResourceDictionaries dictionaries
    ) {
        loadLogicalTablesWithGranularities(nameGroupMap, validGrains, dictionaries);
    }
     ...
     */
    @Deprecated
    public void loadLogicalTable(
            String logicalTableName,
            TableGroup nameGroup,
            Set<TimeGrain> validGrains,
            ResourceDictionaries dictionaries
    ) {
        loadLogicalTableWithGranularities(logicalTableName, nameGroup, validGrains, dictionaries);
    }
    ...
    protected PhysicalTable loadPhysicalTable(
        ...
        DimensionDictionary dimensionDictionary = dictionaries.getDimensionDictionary();

        // Build the physical table
        PhysicalTable table = buildPhysicalTable(definition, metricNames, dimensionDictionary);

        // Add the table to the dictionary
        LOG.debug("Physical table: {} \n\n" + "Cache: {} ", definition.getName().asName(), table.getColumns());
        dictionaries.getPhysicalDictionary().put(definition.getName().asName(), table);
    ...
    protected PhysicalTable buildPhysicalTable(
        ...
        PhysicalTable physicalTable = new PhysicalTable(definition.getName().asName(), definition.getGrain());
