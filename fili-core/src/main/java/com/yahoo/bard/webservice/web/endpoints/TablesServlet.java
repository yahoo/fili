...
@Path("/tables")
@Singleton
public class TablesServlet extends EndpointServlet {
    ...
    public Response getTablesFullView(
        ...
        try {
            ...
            Stream<TableView> result = fullViewProcessor.formatTables(tablesApiRequest.getTables(), uriInfo).stream();
            Response response = formatResponse(tablesApiRequest, result, "tables", null);
            LOG.debug("Tables Endpoint Response: {}", response.getEntity());
            RequestLog.stopTiming(this);
            return response;
        } catch (JsonProcessingException e) {
