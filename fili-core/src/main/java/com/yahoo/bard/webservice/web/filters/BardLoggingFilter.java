...
public class BardLoggingFilter implements ContainerRequestFilter, ContainerResponseFilter, ClientRequestFilter,
    ...
    public void filter(ContainerRequestContext request, ContainerResponseContext response)
        ...
        b.append("\tlength=").append(request.getProperty(PROPERTY_REQ_LEN)).append("\t");
        b.append(renderUri(request.getUriInfo().getRequestUri())).append("\t");

        b.append("Response: ").append(response.getStatus()).append("\t");
        b.append(response.getStatusInfo()).append("\t");

        Long ts = (Long) request.getProperty(PROPERTY_NANOS);
        if (ts != null) {
            b.append((System.nanoTime() - ts) / MILLISECONDS_PER_NANOSECOND);
        }
        b.append(" ms\t");
        ...
        appendStringHeaders(b, "> ", request.getHeaders().entrySet());
        appendObjectHeaders(b, "< ", response.getHeaders().entrySet());
    ...
    public void filter(ClientRequestContext request, ClientResponseContext response) throws IOException {
        ...
        b.append("ClientRequest: ").append(request.getMethod()).append("\t");
        b.append(request.getUri().toASCIIString()).append("\t");

        appendObjectHeaders(b, "> ", request.getHeaders().entrySet());
        appendStringHeaders(b, "< ", response.getHeaders().entrySet());

        b.append(response.getStatusInfo()).append(", length=").append(response.getLength()).append(" ");

        Long ts = (Long) request.getProperty(PROPERTY_NANOS);
        if (ts != null) {
            b.append((System.nanoTime() - ts) / MILLISECONDS_PER_NANOSECOND).append(" ms\t");
        }

        LOG.info(b.toString());
        RequestLog.stopTiming(CLIENT_TOTAL_TIMER);
        // This is not essential here. I leave it to see if a call to these filter methods ever shows up
        LOG.info(RequestLog.export());
    }
    ...
    public void aroundWriteTo(WriterInterceptorContext writerInterceptorContext) throws IOException {
        LengthOfOutputStream stream;
        stream = (LengthOfOutputStream) writerInterceptorContext.getProperty(PROPERTY_OUTPUT_STREAM);

        try {
            writerInterceptorContext.proceed();

            if (stream != null) {
                LOG.info(stream.getStringBuilder().toString());
            }
        } catch (Error | RuntimeException | IOException e) {
            // catch all and rethrow after logging
            if (stream != null) {
                LOG.info("{}\t{}", stream.getStringBuilder().toString(), e.toString());
            } else {
                LOG.info("{}", e.toString());
            }
            throw e;
    ...
    /**
     * Computes and returns length of response stream.
     */
    static private class LengthOfOutputStream extends OutputStream {
        ...
        LengthOfOutputStream(StringBuilder b, OutputStream os) {
            this.b = b;
            this.ostream = os;
        }
