...
public class PartialDataHandler {
    ...
    public SimplifiedIntervalList findMissingRequestTimeGrainIntervals(
        ...
        SimplifiedIntervalList requestedIntervals = new SimplifiedIntervalList(apiRequest.getIntervals());
        Granularity granularity = apiRequest.getGranularity();

        SimplifiedIntervalList availableIntervals = physicalTables.stream()
                .map(table -> getAvailability(table, getColumnNames(apiRequest, query)))
                .flatMap(SimplifiedIntervalList::stream)
                .collect(SimplifiedIntervalList.getCollector());


        SimplifiedIntervalList simplifiedRequestedIntervals = new SimplifiedIntervalList(requestedIntervals);
        ...
        if (granularity instanceof AllGranularity && !missingIntervals.isEmpty()) {
            missingIntervals = simplifiedRequestedIntervals;
        }
