...
public class TablesApiRequest extends ApiRequest {
     ...
     * @param tablesServlet  The table servlet that created this Api Requests
     *
    ...
    public TablesApiRequest(
            ...
            @NotNull String page,
            UriInfo uriInfo,
            TablesServlet tablesServlet
    ) {
        super(format, perPage, page, uriInfo);

        this.tables = generateTables(tableName, tablesServlet.getLogicalTableDictionary());

        if (tableName != null && granularity != null) {
            this.granularity = generateGranularity(granularity, tablesServlet.getTimeContext());
            this.table = generateTable(tableName, this.granularity, tablesServlet.getLogicalTableDictionary());
        } else {
            this.table = null;
