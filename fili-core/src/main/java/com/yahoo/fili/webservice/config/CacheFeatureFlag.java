...
public enum CacheFeatureFlag implements FeatureFlag {
    ...
    public boolean isOn() {
        ...
        if (FiliFeatureFlag.DRUID_CACHE.isSet() || FiliFeatureFlag.DRUID_CACHE_V2.isSet()) {
            // no cache
            if (this.value.equals("NoCache")) {
                return ! FiliFeatureFlag.DRUID_CACHE.isOn();
            }

            return (this.value.equals("Ttl")
                    && !FiliFeatureFlag.DRUID_CACHE_V2.isOn()
                    && FiliFeatureFlag.DRUID_CACHE.isOn()
            )
