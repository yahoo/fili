...
package com.yahoo.fili.webservice.async.broadcastchannels;
...
/**
 * Fili may exist in a system where multiple Fili hosts exist, it is possible that a user may submit a query on one box,
 * and then come back later and perform a long poll on a separate box. Therefore, we need a mechanism to notify all the
 * Fili boxes when a PreResponse is stored in the PreResponseStore by any of the Fili boxes.
 * <p>
...
public interface BroadcastChannel<T> extends Closeable {
    ...
    /**
     * Publish is used to publish a message from a Fili box.
     *
     ...
     * emits all the notifications passed to it by Fili instances in the cluster.
     *
     * @return An unbounded stream of notifications from Fili instances
     */
