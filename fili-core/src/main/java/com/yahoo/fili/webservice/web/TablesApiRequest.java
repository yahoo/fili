...
import static com.yahoo.fili.webservice.web.ErrorMessageFormat.EMPTY_DICTIONARY;
import static com.yahoo.fili.webservice.web.ErrorMessageFormat.TABLE_GRANULARITY_MISMATCH;
...
public class TablesApiRequest extends ApiRequest {
    ...
    public TablesApiRequest(
        ...
        this.tables = generateTables(tableName, filiConfigResources.getLogicalTableDictionary());

        if (tableName != null && granularity != null) {
            this.granularity = generateGranularity(granularity, filiConfigResources.getGranularityParser());
            this.table = generateTable(tableName, this.granularity, filiConfigResources.getLogicalTableDictionary());
        } else {
            this.table = null;
