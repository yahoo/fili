...
class FiliLoggingFilterSpec extends Specification {

    @Subject FiliLoggingFilter filter = new FiliLoggingFilter()
    ...
    ContainerRequestContext requestContext = Stub(ContainerRequestContext) {
            ...
            getUriInfo() >> Stub(UriInfo) {
                getRequestUri() >> new URI(
                        "https://fili.fili.com/v1/public/table/grain?metrics=blah&dateTime=2017-01-01/2018-01-01"
                )
            }
    }
    ...
    @Unroll
    def "The FiliLoggingFilter successfully adds #requestId to MDC"() {
        given: "A ContainerRequestContext with the specified requestId"
        fakeHeaders.put(FiliLoggingFilter.X_REQUEST_ID_HEADER, [requestId])
    ...
    def "The FiliLoggingFilter is null safe"() {
        given: "A ContainerRequestContext with a null requestId"
        fakeHeaders.put(FiliLoggingFilter.X_REQUEST_ID_HEADER, [null])
    ...
    @Unroll
    def "The FiliLoggingFilter does not add a malformed requestId '#requestId' to MDC"() {
