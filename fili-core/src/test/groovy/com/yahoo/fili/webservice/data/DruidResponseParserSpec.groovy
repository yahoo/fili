...
class DruidResponseParserSpec extends Specification {
    ...
    def setup() {
        ...
        dimensionDictionary.findByApiName("ageBracket").with {
            addDimensionRow(FiliDimensionField.makeDimensionRow(it, "1", "1"))
            addDimensionRow(FiliDimensionField.makeDimensionRow(it, "4", "4"))
            addDimensionRow(FiliDimensionField.makeDimensionRow(it, "", "unknown"))
        }

        dimensionDictionary.findByApiName("gender").with {
            addDimensionRow(FiliDimensionField.makeDimensionRow(it, "u", "u"))
            addDimensionRow(FiliDimensionField.makeDimensionRow(it, "f", "u"))
        }
    ...
    def "parse group by with numeric metrics only into a ResultSet"() {
        given:
        DimensionField description = FiliDimensionField.DESC
        ...
        firstResult.getDimensionRow(unknownColumn)?.get(FiliDimensionField.ID) == "foo"
        ...
        resultWithNullDimensionKey.getDimensionRow(ageColumn)?.get(FiliDimensionField.ID) == ""
        resultWithNullDimensionKey.getDimensionRow(ageColumn)?.get(FiliDimensionField.DESC) == "unknown"
    }
    ...
    def "parse sample top N ResultSet with only numeric metrics"() {
        ...
        result.getDimensionRow((DimensionColumn) schema.columns.toArray()[1])?.get(FiliDimensionField.DESC) == "4"

        and:
        Result resultWithNullDimensionKey = resultSet.get(2)
        resultWithNullDimensionKey.getDimensionRow((DimensionColumn) schema.columns.toArray()[1])?.get(FiliDimensionField.ID) == ""
        resultWithNullDimensionKey.getDimensionRow((DimensionColumn) schema.columns.toArray()[1])?.get(FiliDimensionField.DESC) ==
                "unknown"
    }
    ...
    def "parse lookback with groupBy datasource into a Resultset"() {
        ...
        and:
        Result firstResult = resultSet.get(0)
        firstResult.getDimensionRow(genderColumn)?.get(FiliDimensionField.DESC) == "u"
        firstResult.getDimensionRow(ageColumn)?.get(FiliDimensionField.DESC) == "4"
        firstResult.getDimensionRow(unknownColumn)?.get(FiliDimensionField.DESC) == ""
        firstResult.getDimensionRow(unknownColumn)?.get(FiliDimensionField.ID) == "foo"
        firstResult.getMetricValueAsNumber(pageViewsColumn) == 1 as BigDecimal
