...
class DimensionStoreKeyUtilsSpec extends Specification {
    ...
    def setupSpec() {
        originalCaseSensitiveKeysEnabled = FiliFeatureFlag.CASE_SENSITIVE_KEYS.isOn()
    }

    def cleanupSpec() {
        FiliFeatureFlag.CASE_SENSITIVE_KEYS.setOn(originalCaseSensitiveKeysEnabled)
    }
    ...
    def "getRowKey #preserves case for #param when CASE_SENSITIVE_KEYS_ENABLED is #flagState"() {
        ...
        setup:
        originalFlagState = FiliFeatureFlag.CASE_SENSITIVE_KEYS.isOn()
        FiliFeatureFlag.CASE_SENSITIVE_KEYS.setOn(flagState)

        expect:
        DimensionStoreKeyUtils.getRowKey(rowName, rowValue) == expectedValue

        cleanup:
        FiliFeatureFlag.CASE_SENSITIVE_KEYS.setOn(originalFlagState)
