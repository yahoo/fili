...
import com.yahoo.fili.webservice.web.PageNotFoundException
import com.yahoo.fili.webservice.web.responseprocessors.LoggingContext
    ...
    Map buildDimensionData(int numDimensions) {
        (1..numDimensions).collectEntries() {
            Dimension dimension = new KeyValueStoreDimension(
                    ...
                    "dimension$it" as String,
                    new LinkedHashSet<DimensionField>([FiliDimensionField.ID, FiliDimensionField.DESC]),
                    store,
                    searchProvider
            )
            DimensionColumn column = new DimensionColumn(dimension)
            DimensionField idField = FiliDimensionField.ID
            DimensionField descField = FiliDimensionField.DESC
