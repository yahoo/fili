...
import com.yahoo.fili.webservice.util.DateTimeFormatterFactory
...
class ResponseSpec extends Specification {
    ...
    ResultSet buildTestResultSet(Map<MetricColumn, Object> metricValues, Set<MetricColumn> requestedMetrics) {
        ...
        Map<DimensionColumn, DimensionRow> dimensionRows1 = [
                (dimensionColumns[0]): FiliDimensionField.makeDimensionRow(dimensionColumns[0].dimension, "ymail", "yahoo, mail"),
                (dimensionColumns[1]): FiliDimensionField.makeDimensionRow(dimensionColumns[1].dimension, "mob", """mobile " desc.."""),
                (dimensionColumns[2]): FiliDimensionField.makeDimensionRow(dimensionColumns[2].dimension, "US", "United States")
        ] as LinkedHashMap

        Map<DimensionColumn, DimensionRow> dimensionRows2 = [
                (dimensionColumns[0]): FiliDimensionField.makeDimensionRow(dimensionColumns[0].dimension, "ysports", "yahoo sports"),
                (dimensionColumns[1]): FiliDimensionField.makeDimensionRow(dimensionColumns[1].dimension, "desk", """desktop ," desc.."""),
                (dimensionColumns[2]): FiliDimensionField.makeDimensionRow(dimensionColumns[2].dimension, "IN", "India")
        ] as LinkedHashMap

        Map<Dimension, Set<DimensionField>> defaultDimensionFieldsToShow = [
                (dim1): dimensionFields,
                (dim2): dimensionFields,
                (dim3): [FiliDimensionField.DESC] as Set // to show only description
        ]
    ...
    def "test CSV response header ordering"() {
        ...
        for (String it: dimensionNames) {
            ...
            if (it == "pty_country") {
                ...
                defaultDimensionFieldsToShow.put(dimension, [FiliDimensionField.DESC] as Set)
            } else {
        ...
        Map<DimensionColumn, DimensionRow> dimensionRows = [
                (dimensionColumns[0]): FiliDimensionField.makeDimensionRow(dimensionColumns[0].dimension, "platform", "yahoo, mail"),
                (dimensionColumns[1]): FiliDimensionField.makeDimensionRow(dimensionColumns[1].dimension, "platform_version", "yahoo, mail"),
                (dimensionColumns[2]): FiliDimensionField.makeDimensionRow(dimensionColumns[2].dimension, "product", "yahoo, mail"),
                (dimensionColumns[3]): FiliDimensionField.makeDimensionRow(dimensionColumns[3].dimension, "product_family", "yahoo, mail"),
                (dimensionColumns[4]): FiliDimensionField.makeDimensionRow(dimensionColumns[4].dimension, "pty_country", "yahoo, mail"),
                (dimensionColumns[5]): FiliDimensionField.makeDimensionRow(dimensionColumns[5].dimension, "user_state", "yahoo, mail")
        ] as LinkedHashMap
