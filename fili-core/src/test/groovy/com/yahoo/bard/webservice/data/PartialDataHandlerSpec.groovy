...
class PartialDataHandlerSpec extends Specification {
    ...
    SimplifiedIntervalList unionAllColumns = new SimplifiedIntervalList([new Interval("2014-07/2014-07-29")]);
    SimplifiedIntervalList intersectedAllColumns = new SimplifiedIntervalList([new Interval("2014-07-07/2014-07-29")])

    Dimension dim1, dim2, dim3
    Set<PhysicalTable> tables = [new PhysicalTable("basefact_network", DAY)] as Set
    ...
    def setup() {
        ...
        innerQuery.getDataSource() >> { new TableDataSource<GroupByQuery>("basefact_network") }
        ...
        groupByQuery.getDataSource() >> { new QueryDataSource<GroupByQuery>(innerQuery) }
        ...
        SegmentMetadata segments = new SegmentMetadata(
                [(dim1.druidName): buildIntervals(["2014-07-01/2014-07-09","2014-07-11/2014-07-29"]) as LinkedHashSet,
                 (dim2.druidName): buildIntervals(["2014-07-01/2014-07-29"]) as LinkedHashSet,
                 (dim3.druidName): buildIntervals(["2014-07-01/2014-07-29"]) as LinkedHashSet],
    ...
    def "Given a weekly request make sure partiality rounds to the request weeks"() {
        ...
        expect:
        expectedIntervals == partialDataHandler.findMissingRequestTimeGrainIntervals(apiRequest, groupByQuery, tables)
    }
    ...
    def "Check that partial data on granularity 'all' with some partial intervals reports ALL intervals missing"() {
        ...
        expect:
        requestedIntervals == partialDataHandler.findMissingRequestTimeGrainIntervals(apiRequest, groupByQuery, tables)
    }
