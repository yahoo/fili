...
class NoOpSearchProviderSpec extends Specification {
    ...
    def setup() {
        ...
        keyValueStoreDimension = new KeyValueStoreDimension("other", "other", "other-description",dimensionFields, keyValueStore, noOpSearchProvider)
    ...
    def "findAllDimensionRowsByField using key field returns expected row"() {

        given: "The dimension key field, the value, and an expected row"
        DimensionField keyField = keyValueStoreDimension.getKey()
        String value = "row1"
        DimensionRow expectedRow = BardDimensionField.makeDimensionRow(keyValueStoreDimension, "row1", "row1")

        expect: "We get the expected row when we find the value by the key field"
        noOpSearchProvider.findAllDimensionRowsByField(keyField, value) == [expectedRow] as Set<DimensionRow>
    }

    def "findAllDimensionRowsByField using other field returns expected rows"() {

        given: "The dimension desc field, the value, and an expected row"
        DimensionField descField = keyValueStoreDimension.getFieldByName("desc")
        String value = "row1Desc"
        DimensionRow expectedRow = BardDimensionField.makeDimensionRow(keyValueStoreDimension, "row1Desc", "row1Desc")

        expect: "We get the expected row when we find the value by the desc field"
        noOpSearchProvider.findAllDimensionRowsByField(descField, value) == [expectedRow] as Set<DimensionRow>
    }
