...
class ThetaSketchIntersectionReportingSpec extends Specification {
    ...
    def "When the format of the metric filter is invalid, BadApiRequestException is thrown"(){
        when:
        new DataApiRequest().generateLogicalMetrics("bcookies(AND(country|id-in[US,IN]property|id-in[14,125]))", resources.metricDict, resources.dimensionDict, resources.table)
    ...
    def "When the API query contains duplicate metrics, BadApiRequestException is thrown"(){
        when:
        new DataApiRequest().generateLogicalMetrics("bcookies,bcookies(AND(country|id-in[US,IN],property|id-in[14,125]))", resources.metricDict, resources.dimensionDict, resources.table)

        then:
        Exception e = thrown(BadApiRequestException)
        e.message == ErrorMessageFormat.DUPLICATE_METRICS_IN_API_REQUEST.format("[bcookies]")
    }
    ...
    def "When metric filter contains 'OR' condition, BadApiRequestException is thrown"(){
        when:
        new DataApiRequest().generateLogicalMetrics("bcookies(OR(country|id-in[US,IN],property|id-in[14,125]))", resources.metricDict, resources.dimensionDict, resources.table)
    ...
    def "When the INTERSECTION_REPORTING flag is enabled and the query contains unfiltered metrics, the Logical Metrics returned are equal to the Logical Metrics from the Metric Dictionary"() {
        Set<LogicalMetric> logicalMetrics = new DataApiRequest().generateLogicalMetrics("pageViews,bcookies", resources.metricDict, resources.dimensionDict, resources.table)
        HashSet<Dimension> expected =
                ["pageViews", "bcookies"].collect { String name ->
    ...
    def "When metric filter contains invalid dimension, BadApiRequestException is thrown"(){
        when:
        new DataApiRequest().generateLogicalMetrics("bcookies(AND(country1|id-in[US,IN],property|id-in[14,125]))", resources.metricDict, resources.dimensionDict, resources.table)
    ...
    def "When filtered metric has no post aggs, the estimate of intersection of its Filtered Aggregators is used as its post agg"(){
        TemplateDruidQuery templateDruidQuery = FieldConverterSupplier.metricsFilterSetBuilder.updateTemplateDruidQuery(
                resources.metricDict.get("regBcookies").templateDruidQuery,
                resources.filterObj,
        ...
        expect:
        ((ThetaSketchEstimatePostAggregation) templateDruidQuery.getPostAggregations().first()).field == resources.bcookieRegBcookiesPostAggregationInterim
    }
    ...
    def "updateTemplateDruidQuery replaces the aggs with filteredAggs and postAggs with intersection or union of its filteredAggs"(){
        ...
        TemplateDruidQuery templateDruidQuery = FieldConverterSupplier.metricsFilterSetBuilder.updateTemplateDruidQuery(
                resources.metricDict.get("bcookies").templateDruidQuery,
                resources.filterObj,
                resources.dimensionDict,
                resources.table,
                new DataApiRequest()
        ...
        setOperationPostAggregation.fields == [resources.bcookieNoYuidPostAggregationInterim, resources.bcookieRegBcookiesPostAggregationInterim]
    }

    def "replacePostAggregation replaces the filedAccesors of bcookie postAgg with intersection of its Filtered Aggregators"(){
        FuzzySetPostAggregation replacedPostAgg =
                (FuzzySetPostAggregation) FieldConverterSupplier.metricsFilterSetBuilder.replacePostAggregation(
                        ...
                        resources.bcookiePostAggregation,
        ...
        expect:
        setOperationPostAggregation.getFields() == [resources.bcookieNoYuidPostAggregationInterim, resources.bcookieRegBcookiesPostAggregationInterim]
    }

    def "getFilteredAggregation returns a set of filteredAggregations for a given aggregation and Filter object"(){
        expect:
        resources.bcookieNoYuidFilteredAggregationSet.first() == new FilteredAggregation(
                "bcookieNoYuid-country_id_in_US_IN",
                "bcookieNoYuid",
                resources.bcookieNoYuidAggregation,
                resources.filter
        )
    }
    ...
    def "When API request contains filtered metrics, the Logical Metric returned by generateLogicalMetrics is filtered and therefore not equal to the Logical Metric from the Metric dictionary "(){
        LinkedHashSet<LogicalMetric> logicalMetrics =  new DataApiRequest().generateLogicalMetrics("bcookies(AND(country|id-in[US,IN],property|id-in[14,125]))", resources.metricDict, resources.dimensionDict, resources.table)

        HashSet<Dimension> expected =
                ["bcookies"].collect { String name ->
                    LogicalMetric metric = resources.metricDict.get(name)
                    assert metric?.name == name
                    metric
                }
    ...
    def "An exception is thrown when validateMetrics is passed an intersection expression using invalid metrics"(){
        LinkedHashSet<LogicalMetric> logicalMetrics =  new DataApiRequest().generateLogicalMetrics("regBcookies(AND(country|id-in[US,IN],property|id-in[14,125]))", resources.metricDict, resources.dimensionDict, resources.table)

        when:
        new DataApiRequest().validateMetrics(logicalMetrics,resources.table)

        then:
        String expectedMessage = "Requested metric(s) '[regBcookies]' are not supported by the table 'NETWORK'."
        Exception e = thrown(BadApiRequestException)
