...
import static com.yahoo.bard.webservice.config.BardFeatureFlag.LIMIT_SPEC_ON_PAGINATION
...
class DataPaginationSpecWithLimit extends BaseDataServletComponentSpec {

    static final int ROWS_PER_PAGE = 3

    //Total number of rows returned by our fake Druid query
    static final int NUM_ALL_RESULTS = 9
    @Shared boolean limitSpecOnPagination

    def setupSpec() {
        limitSpecOnPagination = LIMIT_SPEC_ON_PAGINATION.isOn();
        LIMIT_SPEC_ON_PAGINATION.setOn(true)
    }

    def cleanupSpec() {
        LIMIT_SPEC_ON_PAGINATION.setOn(limitSpecOnPagination)
    }
    @Override
     ...
     * @return The query parameters under test
     * @see DataPaginationSpecWithLimit#getQueryParams
     */
    ...
    String getFakeDruidEventResponse(int numRows) {
        List druidResults = (1..numRows).collect {
            """{
                "timestamp": "${"2014-09-${it.toString().padLeft(2, "0")}T00:00:00.000Z"}",
                "event" : {
    ...
    def "Page #pageParam of #numPages with the correct headers and #rowsPerPage rows per page of data, is returned"() {
        ...
        String druidResponse = pageParam.equals("last") ? getFakeDruidResponse(numPages * rowsPerPage) : getFakeDruidEventResponse(numPages * rowsPerPage)
    ...
    def "All rows are returned when #perPage rows per page are requested, but there are only #numRequests rows"() {
        given: "A known Druid response, and the expected API response"
        String druidResponse = getFakeDruidEventResponse(numRequests)
        validateJson(druidResponse)
     ...
     * @see DataPaginationSpecWithLimit#getExpectedApiResponse
     */
