...
/**
 * Tests for the TimeContext class.
 */
class StandardTimeContextSpec extends Specification {
    ...
    def "check standard time context parses (#name) into granularity #expected "() {
        expect:
        timeContext.parseGranularity(name) == expected
    ...
    def "check standard time context parses (#name, UTC) into zoned granularity #expected "() {
        expect:
        timeContext.parseGranularity(name, UTC) == expected
    ...
    def "check invalid granularity creates error"() {
        ...
        when:
        timeContext.parseGranularity(timeGrainName)
