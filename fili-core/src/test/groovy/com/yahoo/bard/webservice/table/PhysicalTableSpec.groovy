...
class PhysicalTableSpec extends Specification {
    ...
    def setupSpec() {
        physicalTable = new PhysicalTable("test table", DAY)
        dimension = new KeyValueStoreDimension("dimension", "druidDim", null, [BardDimensionField.ID] as LinkedHashSet, MapStoreManager.getInstance("dimension"), ScanSearchProviderManager.getInstance("apiProduct"))
    ...
    def "test reset columns correctly initializes"() {
        ...
        when:
        table = new PhysicalTable(name, YEAR)
