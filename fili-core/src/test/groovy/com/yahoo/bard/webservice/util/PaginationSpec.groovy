...
class PaginationSpec extends Specification {
    ...
    def setupSpec(){
        //first page
        pagination1 = new Pagination(CONTENT, new PaginationParameters(ROWS_PER_PAGE, 1))
        // second page
        pagination2 = new Pagination(CONTENT, new PaginationParameters(ROWS_PER_PAGE, 2))
        // last page
        pagination3 = new Pagination(CONTENT, new PaginationParameters(ROWS_PER_PAGE, NUM_PAGES))
    }
    ...
    def "An exception is thrown when page requested is #pagePastTheLast and number of pages is #numPages"(){
        ...
        new Pagination<Integer>(CONTENT, new PaginationParameters(ROWS_PER_PAGE, pagePastTheLast))
    ...
    def "When the result set is empty, and we request a page other than the first, we get a PageNotFoundError" () {
        ...
        new Pagination<Integer>(Collections.emptySet(), new PaginationParameters(ROWS_PER_PAGE, 2))
    ...
    def "When #rowsPerPage are requested with only #rows rows of data, there is only one page, with all the results"() {
        ...
        Pagination<Integer> pagination = new Pagination<>(results, new PaginationParameters(rowsPerPage, 1))
