...
class LuceneSearchProviderSpec extends SearchProviderSpec<LuceneSearchProvider> {
    ...
    void childSetup() {
        ...
        rowLimit = searchProvider.maxResultsWithoutFilters
    }

    @Override
    void childCleanup() {
        searchProvider.maxResultsWithoutFilters = rowLimit
    }
    ...
    def "findAllDimensionRows doesn't throw an exception when the real cardinality is less than the limit "() {
        given:
        searchProvider.maxResultsWithoutFilters = dimensionRows.size() + 1

        expect:
        searchProvider.findAllDimensionRows() == dimensionRows as Set
    }

    def "findAllDimensionRows throws RowLimitReachedException exception when the real cardinality is more than the limit"() {
        given:
        searchProvider.setMaxResultsWithoutFilters(1)
    ...
    def "findAllOrderedDimensionRows doesn't throw an exception when the real cardinality is less than the limit "() {
        given:
        searchProvider.setMaxResultsWithoutFilters(100)
