...
class DruidResponseSerializerSpec extends Specification {
    ...
    @Unroll
    def "Serialize Timeseries with #keys and #values"() {
        setup:
    ...
    @Unroll
    def "Serialize GroupBy with #keys and #values"() {
        setup:
        DruidResponse druidResponse = new DruidResponse();
    ...
    @Unroll
    def "Serialize TopN with #keys and #values"() {
        setup:
        DruidResponse druidResponse = new DruidResponse();
        TopNResultRow row = new TopNResultRow(DATE_TIME)
        for (int i = 0; i < keys.size(); i++) {
            row.add(keys.get(i), values.get(i))
        }
        druidResponse.add(row)
        ...
        asList(key1, key2) | asList(1, 2)    | """[{"timestamp":"2017-06-13T00:00:00.000Z","event":{"key1":1,"key2":2}}]"""
    }
}
