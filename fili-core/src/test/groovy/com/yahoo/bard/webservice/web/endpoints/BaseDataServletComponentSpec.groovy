...
abstract class BaseDataServletComponentSpec extends Specification {
    ...
    private static TimeContext timeContext = new StandardTimeContext();
    ...
    def setup() {
        ...
        populatePhysicalTableCacheIntervals(jtb, interval)
    }
    ...
    static def populatePhysicalTableCacheIntervals(JerseyTestBinder jtb, Interval interval) {
        def intervalSet = [interval] as Set

        // for all tables in physical table
        PhysicalTableDictionary physicalTableDictionary = jtb.configurationLoader.physicalTableDictionary
        physicalTableDictionary.each { String tableName, PhysicalTable table ->
            Map<String, Set<Interval>> dimensionIntervals = [:]
            Map<String, Set<Interval>> metricIntervals = [:]

            table.getColumns(MetricColumn.class).each {
                metricIntervals.put(it.name, intervalSet)
            }
            table.getColumns(DimensionColumn.class).each { DimensionColumn it ->
                dimensionIntervals.put(it.getDimension().getDruidName(), intervalSet)
            }
            SegmentMetadata segments = new SegmentMetadata(dimensionIntervals, metricIntervals)
            // set new cache
            table.resetColumns(segments, jtb.configurationLoader.dimensionDictionary)
        }
    }
    ...
    public static String getTimeGrainString(String name = "day", String dateTimeZone = "UTC") {
        ...
        return MAPPER.writeValueAsString(timeContext.parseGranularity(name, timeZone))
    }
}
