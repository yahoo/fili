...
class KeyValueStoreDimensionSpec extends Specification {
    ...
    def setupKVStore() {
        ...
        kvsDimension = new KeyValueStoreDimension("platform", "platform-druid", "platform-description", dimensionFields, keyValueStore, ScanSearchProviderManager.getInstance("platform"))
    ...
    def "findAllDimensionRowsByField returns correct row"() {
        expect:
        searchProvider.findAllDimensionRowsByField(BardDimensionField.ID, "row1") == [dimensionRow1] as Set
        searchProvider.findAllDimensionRowsByField(BardDimensionField.DESC, "this is a row") == [dimensionRow1, dimensionRow2] as Set
    }

    def "findAllDimensionRowsByField returns empty for bad row"() {
        expect:
        searchProvider.findAllDimensionRowsByField(BardDimensionField.ID, "bad") == [] as Set
        searchProvider.findAllDimensionRowsByField(BardDimensionField.DESC, "this is a bad row") == [] as Set
        searchProvider.findAllDimensionRowsByField(BardDimensionField.ID, null) == [] as Set
        searchProvider.findAllDimensionRowsByField(BardDimensionField.DESC, null) == [] as Set
    }
    ...
    def "Change DimensionRow desc"() {
        ...
        then:
        searchProvider.findAllDimensionRowsByField(BardDimensionField.ID, "row6") == [dimensionRow6] as Set
    }
