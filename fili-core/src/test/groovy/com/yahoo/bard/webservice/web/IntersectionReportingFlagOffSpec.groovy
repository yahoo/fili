...
class IntersectionReportingFlagOffSpec extends Specification {
    ...
    def setup() {
        ...
        [ "locale", "one", "two", "three" ].each { String name ->
            KeyValueStoreDimension keyValueStoreDimension = new KeyValueStoreDimension(name, "druid-"+name, "desc"+name, dimensionFields, MapStoreManager.getInstance(name), ScanSearchProviderManager.getInstance(name))
