...
class DefaultPhysicalTableResolverSpec  extends Specification {
    ...
    static final TimeContext context = new StandardTimeContext()
    ...
    def staticInitialize() {
        ...
        resolver = new DefaultPhysicalTableResolver(new PartialDataHandler())
    ...
    def "#table.name matches with zoned #grainName, #dimensions"() {
        setup:
        Granularity granularity = context.parseGranularity(grainName, UTC)
        TemplateDruidQuery query = buildQuery(queryPrototype);
    ...
    def "#table.name matches with #grain, #metrics"() {
        setup:
        Set<String> columnNames = dimSet1.collect({it.druidName})
