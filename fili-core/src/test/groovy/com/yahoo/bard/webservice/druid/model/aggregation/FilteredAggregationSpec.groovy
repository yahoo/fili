...
class FilteredAggregationSpec extends Specification{
    ...
    def setup() {
        Set<ApiMetricName> metricNames = ["BCOOKIE","BCOOKIE_NO_YUID"]
        ...
        MetricInstance bcookieNoYuidSketchPm = new MetricInstance("BCOOKIE_NO_YUID",sketchCountMaker,"BCOOKIE_NO_YUID_SKETCH")
        LogicalMetric bcookieNoYuidSketch = bcookieNoYuidSketchPm.make()

        metricAgg = bcookieNoYuidSketch.getTemplateDruidQuery().getAggregations().first()
        ...
        filteredAgg = new FilteredAggregation("BCOOKIE_NO_YUID-114_127", metricAgg.getFieldName(), metricAgg, filter1)
        filteredAgg2 = new FilteredAggregation("BCOOKIE_NO_YUID-114_127", genderDependentMetricAgg.getFieldName(), genderDependentMetricAgg, filter1)
    }
    ...
    def "test the filtered aggregator constructor" (){
        expect:
        filteredAgg.getFieldName() == "BCOOKIE_NO_YUID_SKETCH"
        filteredAgg.getName() == "BCOOKIE_NO_YUID-114_127"
        filteredAgg.getFilter() == filter1
        filteredAgg.getAggregation() == metricAgg.withName("BCOOKIE_NO_YUID-114_127")
    }
    ...
    def "test FilteredAggregation withName method" (){
        FilteredAggregation newFilteredAggregation = filteredAgg.withName("BCOOKIE_NO_YUID-US_IN");

        expect:
        newFilteredAggregation.getFieldName() == filteredAgg.getFieldName()
        newFilteredAggregation.getName() == "BCOOKIE_NO_YUID-US_IN"
        newFilteredAggregation.getFilter() == filteredAgg.getFilter()
        newFilteredAggregation.getAggregation() == metricAgg.withName("BCOOKIE_NO_YUID-US_IN").withFieldName(filteredAgg.getFieldName())
    }
    ...
    def "test serialization" (){
        expect:
        filteredAgg.getAggregation().getFieldName() == "BCOOKIE_NO_YUID_SKETCH"
        filteredAgg.getAggregation().getName() == "BCOOKIE_NO_YUID-114_127"
    }
}
