...
class ThetaSketchNestedQuerySpec extends Specification {
    ...
    def "Intersection reporting when Logical Metric has nested query"(){
        ...
        nestedQuery.postAggregations.first().name == "bcookies-propertyidin114125countryidinUSIN"
        ...
        updatedNestedPostAgg.name == "bcookies-countryidinUSINpropertyidin114125"
        logicalMetrics.first().templateDruidQuery.aggregations.first().name == "bcookies_sum-countryidinUSINpropertyidin114125"
        logicalMetrics.first().templateDruidQuery.postAggregations.first().name == "dayAvgBcookies"
    }
    ...
    def "metric filter on uniqueIdentifier metric and expect children of unRegBcookies have right sketch operation function"() {
        ...
        List<ThetaSketchSetOperationPostAggregation> unRegBcookieChildren =
                ((ThetaSketchSetOperationPostAggregation) unRegBcookie1.field).fields

        expect:
        unRegBcookieChildren[0].func.equals(SketchSetOperationPostAggFunction.INTERSECT)
        unRegBcookieChildren[1].func.equals(SketchSetOperationPostAggFunction.UNION)
    }
