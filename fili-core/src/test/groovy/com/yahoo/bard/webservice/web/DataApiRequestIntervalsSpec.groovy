...
class DataApiRequestIntervalsSpec extends Specification {
    ...
    static TimeContext timeContext = new StandardTimeContext()
    ...
    def setup() {
        ...
        [ "locale", "one", "two", "three" ].each { String name ->
            keyValueStoreDimension = new KeyValueStoreDimension(name, "druid-"+name, "desc"+name, dimensionFields, MapStoreManager.getInstance(name), ScanSearchProviderManager.getInstance(name))
    ...
    def "check parsing interval #intervalString parses to #parsedStart/#parsedStop"() {
        ...
        new DataApiRequest().generateIntervals(intervalString, concreteApiRequest.generateGranularity(name, timeContext), dateTimeFormatter).size() == 1

        and: "It parses to the interval we expect"
        new DataApiRequest().generateIntervals(intervalString, concreteApiRequest.generateGranularity(name, timeContext), dateTimeFormatter).first() == expectedInterval
    ...
    def "check parsing interval #intervalString parses to #parsedStart/#parsedStop with the use of time periods and macros"() {
        ...
        new DataApiRequest().generateIntervals(
                intervalString,
                concreteApiRequest.generateGranularity(name, timeContext),
                dateTimeFormatter
        ).size() == 1
    ...
    def "check parsing multiple intervals"() {
        ...
        Set<Interval> intervals = new DataApiRequest().generateIntervals(
                interval1 + "," + interval2,
                concreteApiRequest.generateGranularity("day", timeContext),
                dateTimeFormatter
        )
        intervals == expected
