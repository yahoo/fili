...
class DataApiRequestIntervalsSpec extends Specification {
    ...
    @Shared
    Function<TimeGrain, DateTime> dateParser = { it.roundFloor(new DateTime()) }
    ...
    def "check parsing interval #intervalString parses to #parsedStart/#parsedStop"() {
        ...
        DateAndTimeGenerators.INSTANCE.generateIntervals(
                intervalString,
                apiRequest.bindGranularity(name, granularityParser), dateTimeFormatter
        ).size() == 1
    ...
    def "check parsing interval #intervalString parses to #parsedStart/#parsedStop with the use of time periods and macros"() {
        ...
        DateAndTimeGenerators.INSTANCE.generateIntervals(
                intervalString,
                apiRequest.bindGranularity(name, granularityParser),
                dateTimeFormatter
        ).size() == 1
    ...
    def "check parsing multiple intervals"() {
        ...
        Set<Interval> intervals = DateAndTimeGenerators.INSTANCE.generateIntervals(
                interval1 + "," + interval2,
                apiRequest.bindGranularity("day", granularityParser),
                dateTimeFormatter
        )
        intervals == expected
