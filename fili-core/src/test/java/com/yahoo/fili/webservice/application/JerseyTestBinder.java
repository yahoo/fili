...
import com.yahoo.fili.webservice.web.filters.TestLogWrapperFilter;
...
public class JerseyTestBinder {
    ...
    public JerseyTestBinder(boolean doStart, ApplicationState state, Class<?>... resourceClasses) {
        ...
        // Order matters. First check if FiliLoggingFilter is requested
        boolean skipWrapper = false;
        for (Class<?> cls : resourceClasses) {
            if (cls.getSimpleName().equals("FiliLoggingFilter")) {
                skipWrapper = true;
            }
        }

        // If FiliLoggingFilter is not requested, use a wrapper instead to enable logging of the information that is
