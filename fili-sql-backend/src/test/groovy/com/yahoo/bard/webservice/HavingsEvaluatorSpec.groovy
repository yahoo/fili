...
class HavingsEvaluatorSpec extends Specification {
    ...
    def "GetDimensionNames"() {
        ...
        or(equals(DELETED, ONE), lt(ADDED, TWO)) | asList(max(DELETED), min(ADDED)) | "HAVING MAX(`DELETED`) = 1 OR " +
                "MIN(`ADDED`) < 2"
        or(lt(DELETED, ONE), gt(DELETED, TWO)) | asList(max(DELETED), min(DELETED)) | "HAVING MAX(`DELETED`) = 1" +
                " OR MAX(`DELETED`) < 1" //todo seperate test showing explicitly this should fail
    }
}
