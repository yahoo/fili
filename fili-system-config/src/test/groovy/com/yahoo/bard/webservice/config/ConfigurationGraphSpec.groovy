...
class ConfigurationGraphSpec extends Specification {
    ...
    def "Constructor loads vertices and edges correctly"() {
        ...
        expect:
        (GRAPH.vertices.values() as Set) == expectedConfigurations
        GRAPH.edges.keySet() == expectedNames
        GRAPH.edges.get(MODULE_A_NAME) == [MODULE_C_NAME, MODULE_B_NAME]
        GRAPH.edges.get(MODULE_C_NAME) == []
    }
    ...
    def "Module name collision produces error"() {
        ...
        String moduleMessage = ConfigMessageFormat.MODULE_NAME_DUPLICATION.format(MODULE_A_NAME);
     ...
     *
     * If a lists b and c as dependencies, a has precedence, followed by b, followed by c.  If b depends on d, then
     * a will have precedence, then d, then b, then c.
     */
    @Unroll
