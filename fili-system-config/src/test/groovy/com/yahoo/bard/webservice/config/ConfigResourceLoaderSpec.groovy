...
class ConfigResourceLoaderSpec extends Specification {
    ...
    /**
     * Process a regular expression and a source string into a List matches broken out into Lists of groups matched
     *
    ...
    def "test load resources with name finds both module resources from classpath"() {
        ...
        List<List<String>> actual = resources.collect {readMatches(moduleFilePattern, it.toString())}.flatten()
        List<List<String>> expected = expectedResources.collect {readMatches(moduleFilePattern, it.toString())}.flatten()
