...
public class ModuleLoader {
    ...
    private static final String ILLEGAL_CHARACTER_IN_MODULE_NAME = "Illegal character '%c' in module name.";
    ...
    private ConfigurationGraph loadConfigurationGraph() throws SystemConfigException {
        try {
            Stream<Resource> moduleResources = configResourceLoader.loadResourcesWithName(MODULE_CONFIG_FILE_NAME);

            Map<Configuration, String> configurationFileNameMap =
                    moduleResources.collect(
                            Collectors.toMap(configResourceLoader::loadConfigFromResource, Resource::getFilename
                    )
            );
            ...
            throw new SystemConfigException(MODULE_IO_EXCEPTION.format(e.getMessage()));
        }
    }
    ...
    public static void validateModuleName(String name) throws SystemConfigException {
        ...
        // Module names should resemble java identifiers joined by hyphens
        if (! Character.isJavaIdentifierStart(nameChars[0])) {
            String message = String.format(ILLEGAL_CHARACTER_IN_MODULE_NAME, nameChars[0]);
        ...
        for (int i = 1; i < nameChars.length; i++) {
            if (! Character.isJavaIdentifierPart(nameChars[i]) && nameChars[i] != '-') {
                String message = String.format(ILLEGAL_CHARACTER_IN_MODULE_NAME, nameChars[i]);
                LOG.error(INVALID_MODULE_NAME.logFormat(name, message));
                throw new SystemConfigException(INVALID_MODULE_NAME.logFormat(name, message));
            }
        }
